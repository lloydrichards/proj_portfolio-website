...
import { BasicTimeline, CVTimeline } from "../../experimental/019";
...
and squares. Lets have a go at the new(er) `.join()` pattern
in React.
...
managed by React and `useStae()` and then using`useEffect()`  
D3 can take over the manipulation of the svgs inside its box. While I'm sure D3 could build the whole timeline I'm
