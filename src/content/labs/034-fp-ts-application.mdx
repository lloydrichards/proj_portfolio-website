...
import { ShoppingApp } from "../../lab_modules/034";
...
const addProductToCart =
  ...
  (cart: Cart): E.Either<AppError, Cart> =>
    pipe(
      cart,
      E.fromPredicate(
        // check that there is enough items in the cart to remove
        (cart) =>
          pipe(
            cart.items,
      ...
      E.map((cart) => ({
        // remove any items that have a quantity of 0
        ...cart,
        items: pipe(
          cart.items,
...
const updateCart =
  ...
  (product: Product, amount: number): E.Either<AppError, Cart> =>
    pipe(
      // check that the cart is not full
      cart,
      E.fromPredicate(
        (cart) =>
    ...
    O.fold(
      () =>
        pipe(
          ...
          O.fold(
            () => (
              <div className="bg-background grid grid-cols-5 rounded border p-4 shadow-md">
                <p>Loading...</p>
              </div>
            ),
            (error) => (
              <div className="bg-destructive text-destructive-foreground grid grid-cols-5 rounded border p-4 shadow-md">
                <p>{error.type}</p>
      ...
      () => (
        <div className="not-prose bg-background grid grid-cols-5 rounded border p-4 shadow-md">
