...
- **Champion** leads the quest as primary implementer. For a TDD feature, that's
  `@paladin`. For database work, `@oracle` takes the lead.
- **Vanguard** are the core party members essential for completion, `@scout` for
  reconnaissance, `@cleric` for test coverage, `@bard` for documentation.
- **Support** provides assistance and handles secondary concerns, `@champion`
  for code review, `@alchemist` for performance, `@scribe` for git operations.
- **Consultants** are the Wizard agents called for specialized guidance. They're
  read-only by design, they advise but don't implement.
...
flowchart LR
    ...
    Analyze -->|Feature| F["**Champion:** @paladin<br/>**Vanguard:** @scout<br/>**Consultant:** @ts-wizard"]
...
For a typical feature quest, I might form a party with `@paladin` as Champion
(TDD the implementation), `@scout` as Vanguard (map the relevant code), and
`@ts-wizard` as Consultant (type design guidance), with `@champion` providing a
final review. The routing happens once at the start; after that, the party works
through prompt chaining where each agent's output feeds the next.
