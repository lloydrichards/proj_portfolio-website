...
When building agentic applications, there's rarely one "correct" architecture.
The same problem can be solved with radically different approaches, each with
unique trade-offs in cost, speed, and quality.
...
Over a week of experimentation, I built four distinct personas, each
representing a different approach to context and tool use:

1. **The Craftsman** - Tool-assisted chart builder with agentic loops
2. **The Vega** - Minimal architecture using Vega-Lite specs
...
-> output. It focuses on Vega-Lite JSON schemas with limited chart type support
(line/bar only).
...
The Oracle loads the complete SSZ documentation into the system prompt (marked
with Anthropic cache control), enabling direct markup generation without
external tool calls.
...
easily add new patterns - but works great for well-documented APIs.

### The Librarian: RAG-Lite Pattern

The Librarian implements a RAG-lite pattern with `SszResourceToolkit` that
fetches specific documentation chunks on-demand (`readReference`, `readPattern`,
`readDebugging`) from a smaller base context.

**Pattern:** Dynamic knowledge assembly  
**Iterations:** Up to 10  
**Toolkits:** SampleToolkit + SszResourceToolkit  
...
```ts title="CraftsmanService.ts"
export class CraftsmanService extends Effect.Service<CraftsmanService>()(
  "CraftsmanService",
  {
    effect: Effect.gen(function* () {
      const ai = yield* AiService;
      const sampleTools = yield* SampleToolkit;
      const sszTools = yield* SszToolkit;

      return {
        buildChart: (prompt: string) =>
          ai.streamWithTools(prompt, [...sampleTools, ...sszTools]),
      } as const;
    }),
  }
) {}
