...
import {
  ...
  AddSVGExample2,
  AddSVGExample3,
} from "../../experimental/006";

Goign to playing around with SVGs and anime.js. Using this nifty SVG
line creater [SVG Path Builder](https://codepen.io/anthonydugois/pen/mewdyZ)
...
using the `.add({})` There is still a bit of a problem
...
This doesn't work because the `useEffect()` rerenders each
...
Using the `complet: () =>{}` properties I could pass in a
