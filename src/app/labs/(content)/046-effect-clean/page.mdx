...
export const api = {
  ...
  projects: {
    ...
    fetchFeaturedProjects: async () => ApiRuntime.runPromise(getFeaturedProjects),
    ...
    getProjectBySlug: async (slug: string) => ApiRuntime.runPromise(getProject(slug)),
  },
  occupations: {
...
Creating the `ApiRuntime` is a way to use a managed runtime and  able to pass in the `Context`
at the top level of the application. This is then used to run the `Effect` and return a `Promise`
