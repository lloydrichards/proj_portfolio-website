...
want test the `LabApi` which depends on the `FileSystem`:

```ts
//      ┌─── Layer.Layer<LabApi, PlatformError | ImportError | ParseError, never>
const LabApiLayer = LabApi.Default;
...
//      ┌─── Layer.Layer<LabApi, PlatformError | ImportError | ParseError, never>
const MockLabApi = LabApi.DefaultWithoutDependencies.pipe(
  Layer.provide(MockFileSystemLayer),
);
```

I can now use the `MockLabApi` layer to test how the methods function with
