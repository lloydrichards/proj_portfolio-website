...
well as a separate state for the possible direction. Will also be useful to
...
by having a constant that can be changed which effects the speed proportionally.
...
also implemented a lookup table for what paths the item can take. Now the state
...
take place when a certain amount have accumulated, say when 5 pellets are turned
into a product.
